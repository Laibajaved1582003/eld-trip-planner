    ...
    <AnimatePresence>
      {msg && (
        <motion.p
          ...
          className="mt-1 text-xs text-red-600 dark:text-red-400 flex items-center gap-1.5 font-semibold"
        >
          <span className="inline-block w-1.5 h-1.5 rounded-full bg-red-500 shrink-0" aria-hidden="true" />
          {msg}
        </motion.p>
      )}
    </AnimatePresence>
...
export default function TripForm({ onSubmit, isLoading }) {
  ...
  const inputBase =
    ...
    'px-3.5 sm:px-4 py-2.5 sm:py-3 min-h-[44px] text-sm sm:text-base text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 ' +
    ...
    >
      <div className="space-y-3.5 sm:space-y-4">
        ...
        <motion.button
          ...
          className={
            'w-full rounded-xl min-h-[44px] py-3 px-4 sm:px-6 text-sm sm:text-base font-bold text-white tracking-wide ' +
